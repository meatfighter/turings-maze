...
public class TestDemux {
    ...
    private void launch() throws Exception {
        ...
        final Switch[] addressGates = extract(gates, 3, 2417, 9);
        final Switch[] outputGates = extract(gates, 92, 3, 512);

        for (int i = 0; i < 512; ++i) {            
            storeValue(new Switch[][] { addressGates, null }, i);
            emulate(gates, responses, new Reg[0], maze);
            
            int count = 0;
            for (int j = 0; j < 512; ++j) {
                final Switch gate = outputGates[j];                
                if (!gate.red) {
